// Wrap all your chart logic in a function that accepts the data
    function initializeYearlyChart(
        ...
        const commonXAxisOptions = {
            categories: [...yearsCategories], // Ensure yearsCategories is an array
            title: { text: 'Year' },
            type: 'category'
        };
        ...
        if (hasData) {
            var optionsCombined = {
                ...baseChartSettings,
                chart: { ...baseChartSettings.chart, id: "yearlyTrendsChartInstance", type: 'bar' }, 
                series: [
                ...
                tooltip: {
                    ...
                    custom: function({series, seriesIndex, dataPointIndex, w}) { // Use 'series' directly for active series
                        const year = w.globals.labels[dataPointIndex] || (yearsCategories && yearsCategories[dataPointIndex]) || ''; 
                        
                        let html = `<div class="apexcharts-tooltip-title" style="font-family: Helvetica, Arial, sans-serif; font-size: 12px;">${year}</div>`;
            ...
            var combinedChart = new ApexCharts(document.querySelector("#yearlyTrendsChart"), optionsCombined);
            ...
            // Optional: Handle case where #yearlyTrendsChart exists but there's no data to plot
            // You might want to display a message inside the chart div if it's empty
            const chartDiv = document.querySelector("#yearlyTrendsChart");
